...
def _one_nn_cross_pred_rho(source: np.ndarray, target: np.ndarray,
    ...
    if np.std(obs) < 1e-15 or np.std(preds) < 1e-15:
        return None
...
class Auditor:
    ...
    def audit_stationarity(
        ...
        if window_size >= 4:
            ...
            window_vars = [v for v in window_vars if v > 1e-15]
        ...
        if n >= 4:
            ...
            trend_ratio = abs(slope) / max(sigma, 1e-15)
        ...
        if n >= 40:
            try:
                ...
                pass  # best-effort supplementary check; core ADF/KPSS verdict stands
...
def dominant_periodicity(series, times=None, min_n=20, power_threshold=0.30):
    ...
    return {
        "assessable": True,
        "f_dom": f_dom,
        "period_dom": (1.0 / f_dom) if f_dom > 1e-15 else None,
...
def audit_seasonality_confound(
    ...
    for pair in ccm_pairs:
        ...
        if pc["is_high_seasonality"] and pe["is_high_seasonality"]:
            ...
            if f1 > 1e-15 and abs(f1 - f2) / f1 < freq_match_rel_tol:
                issues.append(
