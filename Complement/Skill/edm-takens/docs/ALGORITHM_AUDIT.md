...
> 审计对象：`f:\攻略\研发测试\.skills\edm-takens`（原生项目，非 Web 版本）  
> 审计日期：2026-07-13  
...
| 文件 | 变更类型 | 变更理由 |
|------|----------|----------|
| `f:\攻略\研发测试\.skills\edm-takens\src\_numpy_edm.py` | 修改 | 统一 numpy 回退 CCM 收敛判定与 `ccm_causality_test()`，补全 `spearman_p` 返回字段。 |
| `f:\攻略\研发测试\.skills\edm-takens\src\pipeline.py` | 修改 | 小样本 Hankel 自校正报告校正后比例，并在不可达 `p/q>=10` 时显式提示用户。 |
| `f:\攻略\研发测试\.skills\edm-takens\docs\ALGORITHM_AUDIT.md` | 新建 | 汇总本轮审计发现、修复与剩余风险，作为当前权威审计文档。 |
