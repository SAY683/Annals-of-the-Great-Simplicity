"""
Legacy tests for the old havok_decompose() helper.

The current production class is SovereignHAVOK (tested in
`tests/test_sovereign_havok.py`). This module is kept as a regression
harness for the procedural fallback and pyEDM integration path; it is
not the primary reference for new development.
"""
...
# Resolve skill data path (test file is in tests/, data is in examples/game_analysis/data/)
_SKILL_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_SKILL_SRC = os.path.join(_SKILL_ROOT, 'src')
sys.path.insert(0, _SKILL_SRC)
from _paths import SKILL_DATA as _SKILL_DATA
