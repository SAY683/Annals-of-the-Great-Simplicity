...
def _make_config(csv_path, target_col, selected_vars, q, max_e, auto_fix, intensity=None):
    # S1 修复 (科研严谨性审查): intensity → analysis_type 映射.
    # 原代码忽略 intensity, 始终走默认 "exploratory" (BH FDR).
    # 现 heavy/medium 映射为 confirmatory (Bonferroni), light/auto 保持 exploratory.
    _intensity_to_analysis_type = {
        'light': 'exploratory',
        'auto': 'exploratory',
        'medium': 'confirmatory',
        'heavy': 'confirmatory',
    }
    analysis_type = _intensity_to_analysis_type.get(
        (intensity or 'auto').lower().strip(), 'exploratory')
    print(f"[API] target_col={target_col}, selected_vars={selected_vars}, q={q}, max_e={max_e}, "
          f"auto_fix={auto_fix}, intensity={intensity}, analysis_type={analysis_type}")
