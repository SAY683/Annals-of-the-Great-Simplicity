...
# ── 工具调用分发 ──────────────────────────────────────────
async def call_tool(name: str, args: dict, port: int):
    a = args or {}
    if name == "list_datasets":
        return await _fetch_async(port, "GET", "/api/datasets")
        ...
        if a.get("q"):
            body["q"] = a["q"]
        return await _fetch_async(port, "POST", "/api/analyze", body)
    elif name == "get_job":
        return await _fetch_async(port, "GET", f"/api/analyze/jobs/{a['job_id']}")
    elif name == "list_history":
        return await _fetch_async(port, "GET", "/api/history")
    elif name == "get_history_detail":
        return await _fetch_async(port, "GET", f"/api/history/{a['task_id']}")
    elif name == "health":
        return await _fetch_async(port, "GET", "/api/health")
    else:
        raise ValueError(f"Unknown tool: {name}")


async def _fetch_async(port, method, path, body=None):
    """异步包装（用 anyio.to_thread.run_sync 在 FastAPI 中不阻塞事件循环）"""
    import anyio
    return await anyio.to_thread.run_sync(_local_fetch, port, method, path, body)
...
def create_mcp_router(port: int) -> APIRouter:
    ...
    async def mcp_endpoint(request: Request):
        ...
        if method == "tools/call":
            ...
            try:
                status, j, text = await call_tool(name, args, port)
