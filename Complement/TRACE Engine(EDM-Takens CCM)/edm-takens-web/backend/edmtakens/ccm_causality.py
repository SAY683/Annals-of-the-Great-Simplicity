...
def ccm_causality_test(df, cause_var, effect_var, E, lib_sizes=None,
                        ...
                        force_numpy_backend=False, rng_seed=42):
    """
    ...
    for direction_idx, (col_var, tgt_var) in enumerate([
        ...
        try:
            ccm = _bridge_CCM(
                ...
                showPlot=False,
                force_numpy=force_numpy_backend, rng=rng_seed)
            ...
            results[direction_idx] = {
                ...
                'rhos': rhos.tolist(),
                # S1 修复 (科研严谨性审查 G2): 暴露 ρ 的不确定性指标.
                # rhos 数组是不同 lib_size 下 50 次 bootstrap 的平均 ρ,
                # 其标准差反映 ρ 随 lib_size 变化的波动 (非严格 bootstrap CI,
                # 但可作为收敛稳定性的粗略指标).
                'rho_std': float(np.std(rhos_valid)) if n_valid >= 2 else 0.0,
                'rho_min': float(np.min(rhos_valid)) if n_valid >= 1 else 0.0,
                'rho_max': float(np.max(rhos_valid)) if n_valid >= 1 else 0.0,
                'ccm_raw': ccm,
            }
...
def ccm_batch_test(df, pairs, E, analysis_label: str = 'exploratory',
                    ...
                    lib_sizes=None,
                    strong_direction_rho: float = 0.2,
                    use_surrogate_p: bool = False,
                    n_surrogates: int = 19,
                    surrogate_seed: int = 42) -> dict:
    """
    ...
    use_surrogate_p : bool, default False. If True, compute IAAFT
        surrogate-based p-values for pairs passing the effect-size gate,
        replacing Spearman p in the multiple-comparison correction.
        Addresses the methodology_disclaimer about effect-size gating
        breaking BH's uniform-null assumption. Use for strict
        confirmatory analysis. P1-3.1 修复.
    n_surrogates : int, default 19. Number of IAAFT surrogates per pair
        (19 → p<0.05 resolution; 99 → p<0.01).
    surrogate_seed : int, default 42. Base seed for surrogate generation.
    ...
    for cause, effect in pairs:
        ...
        # P1-3.1 修复 (科研严谨性审查): 可选 IAAFT surrogate p 值路径.
        # 默认 use_surrogate_p=False 保持向后兼容. 启用后, 对 effect-size
        # gate 通过的 pair, 用 IAAFT surrogate 检验得到因果 null 分布的 p 值,
        # 替代 Spearman p 进入 BH/Bonferroni 校正. 这解决了 effect-size gating
        # 破坏 BH uniform-null 假设的方法学限制 (见 methodology_disclaimer).
        # 严格确认性分析应设置 use_surrogate_p=True.
        surrogate_p_value = None
        if use_surrogate_p and effect_size_ok and which is not None:
            try:
                from surrogate_test import iaaft_surrogates
                # 选择测试方向的真实序列 (effect 的 manifold → cause)
                _target_col = effect if which == 'forward' else cause
                _source_col = cause if which == 'forward' else effect
                _real_series = df[_target_col].values
                ...
                _surr_rhos = []
                _surr_failures = 0  # S1-1 修复: 记录失败次数
                for _s in _surrs:
                    _surr_df = df.copy()
                    _surr_df[_target_col] = _s
                    try:
                        _surr_r = ccm_causality_test(_surr_df, cause, effect, E,
                                                     lib_sizes=lib_sizes)
                        _surr_dir = _surr_r[which]
                        if _surr_dir.get('final_rho') is not None:
                            _surr_rhos.append(_surr_dir['final_rho'])
                        else:
                            _surr_failures += 1
                    except Exception as _surr_e:
                        # S1-1 修复 (科研严谨性审查 Round 27): 原 except: pass
                        # 静默吞错, 若多个 surrogate 因数值问题被跳过, p 值
                        # 分辨率下降且可能系统性偏低 (假阳性偏向). 现记录到
                        # stderr 便于调试, 同时累计失败次数用于后续判定.
                        import sys
                        print(f"[ccm_batch_test] surrogate CCM failed (cause={cause}, "
                              f"effect={effect}): {type(_surr_e).__name__}: {_surr_e}",
                              file=sys.stderr)
                        _surr_failures += 1
                # S1-1 修复 (续): 失败比例 > 50% 时拒绝给出 p 值
                # 防止基于过小有效样本 (n_surrogates/2 以下) 给出误导性 p 值.
                # Phipson & Smyth 2010 要求 null 分布有足够样本量, 当多数
                # surrogate 失败时 p 值不可信.
                _surr_total = len(_surrs)
                _fail_ratio = _surr_failures / _surr_total if _surr_total > 0 else 1.0
                if _fail_ratio > 0.5:
                    import sys
                    print(f"[ccm_batch_test] WARNING: {_surr_failures}/{_surr_total} "
                          f"surrogates failed ({_fail_ratio:.0%}); surrogate_p_value "
                          f"set to None (insufficient valid surrogates for reliable p)",
                          file=sys.stderr)
                    surrogate_p_value = None
                    # p_val_gated 保持原 Spearman p 值, 不用 surrogate 替代
                elif len(_surr_rhos) >= 3:
                    _real_rho = which_rho
                    # 单侧检验: real ρ 是否显著高于 surrogate null
                    _count_ge = sum(1 for r in _surr_rhos if r >= _real_rho)
                    surrogate_p_value = float((_count_ge + 1) / (len(_surr_rhos) + 1))
                    # 用 surrogate p 替代 Spearman p 进入校正
                    p_val_gated = surrogate_p_value
            except ImportError:
                pass  # surrogate_test 不可用时回退到 Spearman p
    ...
    # S1 修复 (科研严谨性审查 D4): 集成周期混淆检测 (Secret 10).
    # 原代码仅 advisory, CCM 路径完全不调用 audit_seasonality_confound.
    # 两个共享相同主导频率的序列会得到高 ρ 且"收敛" (Cobey-Baskerville 2016 假阳性).
    # 现在对显著的结果调用周期混淆检测, 在 note 中追加警告.
    # P2-7.3 修复 (科研严谨性审查): 扩展检测范围到 significant_raw 的 pair,
    # 不仅检查 significant_corrected. raw 显著但校正后不显著的 pair 仍可能是
    # 周期混淆导致的假阳性, 检测它们有助于诊断.
    seasonality_warning = ""
    try:
        from edm_auditor import audit_seasonality_confound
        # 构造 data_dict: 只包含参与测试的列
        _data_dict = {col: df[col].values for col in df.columns
                      if col in {p[0] for p in pairs} | {p[1] for p in pairs}}
        # 检查所有 raw 显著的 pair (不仅 corrected 显著的)
        _converged_pairs = [(res['cause'], res['effect'], True)
                            for res in results
                            if res.get('significant_corrected') or res.get('significant_raw')]
        if _converged_pairs:
            _seasonality_finding = audit_seasonality_confound(
                _data_dict, _converged_pairs)
            if _seasonality_finding.verdict == 'WARN':
                seasonality_warning = (
                    f" ⚠ SEASONALITY CONFOUND: {_seasonality_finding.message}. "
                    f"Convergent pairs may share a periodic driver — "
                    f"interpret causal claims with caution."
                )
    except ImportError:
        pass  # edm_auditor 不可用时静默跳过 (CLI 可能独立运行)
    except Exception as _e:
        # P2-8.4 修复: 记录异常信息而非完全静默, 便于调试周期混淆检测本身的问题
        import sys
        print(f"[ccm_batch_test] seasonality confound check advisory failed: {_e}",
              file=sys.stderr)
        pass  # 周期混淆检测是 advisory, 不阻断主流程
    ...
    return {
        ...
        # S1 修复 (科研严谨性审查 C3): 方法学 disclaimer — effect-size 门控改写 p 值语义.
        # BH/Bonferroni 校正假设输入 p 值在 null 下 uniform, 但本实现用 effect-size
        # 门控 (abs(rho)>0.2 才允许 p 进入校正, 否则 p=1.0), 改变了 p 值分布.
        # 这是防止 "近零 ρ + 大样本 → 天文级小 p" 假阳性的必要措施 (见 docstring),
        # 但严格来说不再满足 BH 的 uniform-null 假设. 科研报告应标注此方法学限制.
        # 严格做法: 用 IAAFT surrogate 检验得到因果 null 分布的 p 值 (见 B5).
        "methodology_disclaimer": (
            "Effect-size gating (abs(rho)>0.2) is applied before FDR correction, "
            "which modifies p-value semantics: p-values below the gate threshold "
            "are not strictly uniform under null. This is a necessary anti-false-positive "
            "measure (see docstring) but means BH/Bonferroni correction is applied to "
            "gated p-values, not raw null-uniform p-values. For strict confirmatory "
            "analysis, surrogate-based p-values (IAAFT) are recommended."
        ),
        # S1-2 修复 (科研严谨性审查 Round 27): 显式披露 BH 保证级别.
        # is_strict_confirmatory=True 当且仅当:
        #   1. use_surrogate_p=True (用 IAAFT null 分布, 满足 BH uniform-null 假设)
        #   2. analysis_label in {'confirmatory', 'preregistered'} (Bonferroni 或预注册)
        # 否则为 False, 表示当前 p 值的 BH 校正基于 effect-size gated Spearman p,
        # 不严格满足 BH 假设, 仅适合探索性分析.
        # 下游消费者 (科研报告 / Web 前端 / CLI 输出) 应据此字段选择措辞:
        ...
        "is_strict_confirmatory": bool(
            use_surrogate_p and analysis_label in ('confirmatory', 'preregistered')
        ),
    }
