...
def sensitivity_scan(data, base_E: int, metric_fn: Callable,
    ...
    cv = abs(results['neighbor_std'] / (abs(results['neighbor_mean']) + EPS_VARIANCE))
    results['cv'] = float(cv)
...
def decay_profile_scan(data, E: int, max_tp: int = None,
    ...
    if pos_mask.sum() >= 3:
        ...
        exp_r2 = 1.0 - ss_res / ss_tot if ss_tot > EPS_DISTANCE else 0.0
    out["exp_r2"] = exp_r2
    ...
    if lyap_lambda is not None and lambda_fit is not None and lyap_lambda > EPS_VARIANCE:
