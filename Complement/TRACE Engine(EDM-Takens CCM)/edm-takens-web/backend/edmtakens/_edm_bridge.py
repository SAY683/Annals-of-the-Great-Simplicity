...
def CCM(data, columns, target, E, Tp=0, libSizes=None, sample=30,
        showPlot=False, force_numpy=False, rng=None, **kwargs):
    ...
    P2-4.4 修复 (科研严谨性审查): 新增 force_numpy 和 rng 参数.
    pyEDM 的 CCM `sample` 参数使用其内部 RNG, 不受 np.random.seed 控制,
    导致结果不可严格复现. 设置 force_numpy=True 可强制使用 numpy fallback
    路径 (_np_CCM), 该路径接受 rng 参数, 保障严格可复现性.
    严格确认性分析应设置 force_numpy=True + rng=固定种子.
    """
    if EDM_AVAILABLE and hasattr(data, 'columns') and not force_numpy:
        ...
        cause = arr[:, tgt_idx]
        effect = arr[:, col_idx]
        # P2-4.4 修复: 透传 rng 参数到 _np_CCM, 保障可复现性
        result = _np_CCM(cause, effect, E=E, Tp=Tp,
                         libSizes=libSizes, sample=sample, rng=rng)
        import pandas as pd
        df = pd.DataFrame({
