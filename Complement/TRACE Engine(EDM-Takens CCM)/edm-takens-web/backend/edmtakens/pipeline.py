...
class PipelineConfig:
    ...
    # S2-3 修复 (科研严谨性审查 Round 27): 可复现性 seed 透传字段.
    # 原 run_full_analysis 硬编码 _REPRODUCIBILITY_SEED=42 并用 np.random.seed
    # 污染全局 RNG. 现作为 config 字段, 允许用户自定义, 并供下游 default_rng
    # 创建独立 Generator (surrogate_test, _numpy_edm CCM, final_interpretation
    # 的 Lyapunov divergence 均已支持 rng/seed 参数透传).
    # 注意: pyEDM 后端的 CCM sample 参数使用其内部 RNG, 不受此 seed 控制;
    # 严格可复现需 force_numpy=True (见 _edm_bridge.CCM 的 force_numpy 参数).
...
def run_pipeline(config: PipelineConfig = None,
    ...
    try:
        ...
        # S1-4 修复 (科研披露落地 Round 28): CLI 输出科研披露字段
        # 科研用户通过 CLI 运行分析时, 必须能看到统计保证级别和方法学声明,
        # 以便在论文中正确引用结论. 这是科研级产品的基本披露要求.
        is_strict = ccm_batch.get('is_strict_confirmatory', False)
        if is_strict:
            print(f"    [Guarantee] CONFIRMATORY — FDR-controlled at q=0.05 "
                  f"(IAAFT null, Bonferroni-corrected)")
        else:
            print(f"    [Guarantee] EXPLORATORY — Exploratory FDR estimate "
                  f"(effect-size gated Spearman, BH)")
        disclaimer = ccm_batch.get('methodology_disclaimer')
        if disclaimer:
        ...
        for pres in ccm_batch['pairs']:
            ...
            # S1-4 修复: 标注 out-of-sample vs in-sample 评估模式
            # OOS = Sugihara 2012 严格契约; IN-SAMPLE = library 过小时回退路径 (ρ 可能高估)
            oos_flag = "OOS" if fwd.get('out_of_sample_used') else "IN-SAMPLE"
            # S1-4 修复: 显示 effective_lib_sizes (OOS 模式下 = lib_size // 2)
            eff_sizes = fwd.get('effective_lib_sizes') or fwd.get('lib_sizes', [])
            eff_str = (f"eff_lib={eff_sizes[0]}..{eff_sizes[-1]}"
                       if len(eff_sizes) >= 2 else
                       (f"eff_lib={eff_sizes[0]}" if eff_sizes else "eff_lib=N/A"))
            print(f"    M_{config.target_col} -> {cause}: rho={rho_str} "
                  f"({direction}, {conv}, {oos_flag}, {eff_str}) — {sig}")
...
def run_full_analysis(config: 'PipelineConfig' = None,
    ...
    """
    # S1 修复 (科研严谨性审查 E1/E2): 设置全局随机种子保障可复现性.
    # 原代码 CCM 默认 rng=None (非确定性熵源), 同一输入两次运行得到不同 ρ.
    # 现在在 pipeline 入口设置固定种子, 确保结果可复现.
    #
    # S2-3 修复 (科研严谨性审查 Round 27): 原 np.random.seed() 污染全局 RNG,
    # 可能影响同进程中其他随机操作 (matplotlib 状态, pyEDM 内部 RNG 等).
    # 改为:
    #   1. seed 从 config.reproducibility_seed 读取 (默认 42, 用户可自定义)
    #   2. 全局 np.random.seed 仅作为 pyEDM 后端的兜底 (pyEDM CCM 的 sample
    #      参数使用其内部 RNG, 不接受外部 rng; 全局 seed 是已知的 best-effort
    #      兜底, 见 ccm_causality.py 的 P2-4.4 修复注释)
    #   3. 创建独立 Generator _pipeline_rng, 供支持 rng 透传的子模块使用
    #      (surrogate_test, _numpy_edm.CCM, final_interpretation 的 Lyapunov)
    #   4. 严格可复现: 在 config 中设 force_numpy=True (见 _edm_bridge.CCM)
    _REPRODUCIBILITY_SEED = (
        config.reproducibility_seed if config and hasattr(config, 'reproducibility_seed')
        else 42
    )
    # 全局 seed 作为 pyEDM 兜底 (无法避免, 是 pyEDM 已知限制)
    np.random.seed(_REPRODUCIBILITY_SEED)
    # 独立 Generator 供支持 rng 参数的子模块使用 (不污染全局状态)
    _pipeline_rng = np.random.default_rng(_REPRODUCIBILITY_SEED)
