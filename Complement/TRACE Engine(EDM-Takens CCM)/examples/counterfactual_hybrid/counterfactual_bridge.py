...
class DoWhy14Adapter:
    ...
    def get_confidence_interval(estimate):
        ...
        if hasattr(estimate, 'get_confidence_intervals'):
            ...
            if ci is not None:
                # 可能是 [[low, high]] 或 [low, high]
                if isinstance(ci, (list, tuple)) and len(ci) > 0:
                    inner = ci[0] if isinstance(ci[0], (list, tuple, np.ndarray)) else ci
                    return [float(inner[0]), float(inner[-1])]
        return [float('nan'), float('nan')]

    @staticmethod
    def check_refuted(estimate, refutation, threshold: float = 0.3) -> dict:
        """
        跨版本检查反驳状态。
...
def estimate_sem_from_data(adj_matrix, data, concept_names):
    ...
    对每个子节点 Y，用其所有父节点 X 做 OLS 回归: Y ~ Σβ_i·X_i
    """
    V = len(concept_names)
    coeff = np.zeros((V, V))
...
class TRACE2DoWhy:
    ...
    def aggregate_concepts(self) -> dict:
        ...
        for i, tok in enumerate(self.token_list):
            concept_map[i] = tok if tok in high_freq else "<other>"
    ...
    def build_model(self, data_df=None):
        ...
        edges_filtered = [e for e in edges if _is_valid_concept(e[0]) and _is_valid_concept(e[1])]
        ...
        for src, dst, _ in edges_filtered:
            if _is_valid_concept(src) and _is_valid_concept(dst):
                dot_nodes.add(src)
                dot_nodes.add(dst)
        dot_lines = ["digraph {"]
        ...
        for src, dst, _ in edges:
            ...
            if si is not None and di is not None:
                bin_adj[si, di] = 1
        self.sem_coeff = estimate_sem_from_data(bin_adj, raw_data, self.concept_names)
    ...
    def _simulate_data(self, n_concepts: int, n_samples: int = 100):
        """从邻接矩阵模拟生成观测数据（SEM 数据生成过程）"""
        data = np.zeros((n_samples, n_concepts))
        noise = self.rng.normal(0, 0.1, (n_samples, n_concepts))

        for j in range(n_concepts):
            data[:, j] = noise[:, j]
            for i in range(j):
                if self.concept_adj[i, j] > self.threshold:
                    effect = self.concept_adj[i, j] / 10.0
                    data[:, j] += effect * data[:, i]
    ...
    def refute(self) -> dict:
        ...
        for method_name, label in refuters:
            try:
                ...
                check = DoWhy14Adapter.check_refuted(self.estimate_result, result)
    ...
    def visualize(self, filename: str = "causal_graph", format: str = "png",
        ...
        graphviz_bin_dir : str or None
            graphviz bin 目录路径，如 'G:/AI/Graphviz/bin'
            如果为 None，尝试从 GRAPHVIZ_BIN_DIR 环境变量获取
        ...
        # Windows: 确保 graphviz bin 在 PATH 中
        import os as _os
        bin_dirs_to_try = []
        if graphviz_bin_dir:
            bin_dirs_to_try.append(graphviz_bin_dir)
        env_bin = _os.environ.get('GRAPHVIZ_BIN_DIR', '')
        if env_bin:
            bin_dirs_to_try.append(env_bin)
        # 常见安装位置
        for candidate in ['G:/AI/Graphviz/bin', 'C:/Program Files/Graphviz/bin',
                          'C:/Program Files (x86)/Graphviz/bin']:
            if candidate not in bin_dirs_to_try and _os.path.isdir(candidate):
                bin_dirs_to_try.append(candidate)
    ...
    def report(self) -> str:
        ...
        if self.refutation_results:
            ...
            lines.append("")
            lines.append("| 反驳方法 | 原始效应 | 新效应 | 偏差 | 判定 |")
            lines.append("|---------|---------|--------|------|------|")
            ...
            for name, result in self.refutation_results.items():
                ...
                deviation = abs(new_eff - orig) / (abs(orig) + 1e-10)
                check = getattr(result, '_check', None)
                refuted = check['refuted'] if check else (deviation > 0.3)
                verdict = "⚠️ 反驳" if refuted else "✓ 稳健"
                lines.append(f"| {name} | {orig:.4f} | {new_eff:.4f} "
                             f"| {deviation:.1%} | {verdict} |")
            lines.append("")
...
class SimulationRefutation:
    def __init__(self, new_effect, refuted=False):
        self.new_effect = new_effect
        self._check = {'refuted': refuted, 'deviation': 0.0}
...
class SimulationModel:
    ...
    def refute_estimate(self, identified_estimand, estimate,
        ...
        if method_name == "random_common_cause":
            ...
            return SimulationRefutation(placebo, placebo > estimate.value * 0.2)
