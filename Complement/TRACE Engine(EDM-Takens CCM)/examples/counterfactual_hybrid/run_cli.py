...
环境变量:
    GRAPHVIZ_BIN_DIR    Graphviz bin 目录 (默认: G:/AI/Graphviz/bin)
    TRACE_MODEL_DIR     TRACE 模型目录 (默认: 自动检测)
"""
...
def _setup_graphviz():
    """自动检测并配置 graphviz PATH"""
    candidates = [
        os.environ.get('GRAPHVIZ_BIN_DIR', ''),
        'G:/AI/Graphviz/bin',
...
def cmd_env():
    ...
    for model_name in ['Shehui-LLaMA', 'Shenji-LLaMA']:
        model_dir = SKILL_DIR.parent.parent.parent.parent / "TRACE" / "models" / model_name.lower().replace('-', '_')
        # Try portable copy path
        if not model_dir.exists():
            # Look in portable copy
            for p in [SKILL_DIR.parent.parent.parent,  # up to Complement/
                      Path('G:/git/Annals-of-the-Great-Simplicity-main/Annals-of-the-Great-Simplicity/Complement')]:
                candidate = p / "TRACE Engine(EDM-Takens CCM)" / model_name
                if candidate.exists():
                    model_dir = candidate
                    break
        if model_dir.exists():
