...
def _panel_trace(ax, bridge, card):
    ...
    _hdr(ax, f"🔴 TRACE · ΔNLL分布 [{st}] · max={max(vals):.1f}")
...
def _panel_dowhy(ax, bridge):
    ...
    _hdr(ax, f"🟡 DoWhy+CF · {bridge.treatment}→{bridge.outcome}")
...
def _panel_causallearn(ax, card):
    if card is None:
        ax.text(0.5, 0.5, "causallearn 未运行", ha='center', va='center',
               fontsize=FT['label'])
        _hdr(ax, "⬜ causallearn · 独立验证者 (PC/GES)")
        return
