...
class GroundTruthTests:
    ...
    def _test_lorenz(self):
        ...
        if _PYEDM:
            try:
                ...
                # ROUND27 P2 修复: 原 `except: pass` 静默吞错, 现打印失败原因
                # (常见: 数据过短、列名冲突、pyEDM 内部数值错误), 便于审计回溯.
                print(f"    [verify_algorithms] pyEDM PredictNonlinear 失败, 跳过 EDM 非线性检查: "
                      f"{type(_edm_err).__name__}: {_edm_err}")
    ...
    def _test_logistic(self):
        ...
        def logistic_series(r, n=500, x0=0.2):
            # NOTE: x0=0.2, not 0.5. At r=4.0, x0=0.5 is the map's exact
            # critical point: x1 = 4*0.5*0.5 = 1.0, then
            # x2 = 4*1*(1-1) = 0.0 exactly, and every iterate after that
            # is 0 forever (0 is a fixed point). In exact real arithmetic
            # almost no other starting point ever lands exactly on 0, but
            # 0.5 and 1.0 are both exactly representable in binary
            # floating point, so this "measure-zero" degenerate orbit is
            # hit deterministically in float64. The resulting series
            # (after discarding the x[:200] transient) was a constant-0
            # array, not a chaotic one — SovereignHAVOK correctly reported
            # this as degenerate (std < 1e-12) and, before the fit()
            # NaN-guard fix, explained_var_ came out as 0/0 = NaN, making
            # this "chaotic" test case silently fail every check for a
            # reason that had nothing to do with EDM/HAVOK correctness.
            # x0=0.2 (used throughout the chaos-theory literature for
            # this exact reason) produces a genuine chaotic trajectory at
            # r=4.0 — see docs/CHANGELOG.md.
            x = np.zeros(n); x[0] = x0
...
class InternalConsistencyTests:
    ...
    def run_all(self):
        print("\n" + "=" * 70)
        print(f"  LEVEL 4: Internal Consistency (max {self.max_score} pts)")
        print("=" * 70)
