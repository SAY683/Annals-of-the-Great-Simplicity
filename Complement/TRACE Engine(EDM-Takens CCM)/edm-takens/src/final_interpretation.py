...
def estimate_lyapunov_robust(data, E, dt=1.0):
    ...
    ac = ac[len(ac)//2:]
    # P1-5.3 修复 (科研严谨性审查): 常量序列时 ac[0]=0 导致除零, NaN 传播到
    # np.diff(np.sign(ac - 1/e)) 产生空数组, mean_period 静默 fallback.
    # 同步 enhanced_cross_validate.py 行 109-116 的 ac0 防护, 显式处理零能量.
    ac0 = ac[0] if len(ac) > 0 else 0.0
...
def estimate_lyapunov_lower_bound(data, E, dt=1.0, n_surrogates=19,
    ...
    # P1-4.3 修复 (科研严谨性审查): _divergence_rate_metric 原使用 np.random.choice
    # (全局 RNG), 不受传入 seed 控制 → real_rate 不可复现, is_sig 判定不可复现.
    # 改为接受 rng_seed 参数, 内部创建独立 Generator, 与 surrogate_test.py 的
    # default_rng 改造对齐, 保障 Lyapunov lower bound 判定的可复现性.
    def _divergence_rate_metric(d, rng_seed=None):
        ...
        if N_local <= 12:
            return 0.001
        # P1-4.3 修复: 使用独立 Generator 替代全局 np.random
        _rng = np.random.default_rng(rng_seed)
        for _ in range(10):  # bootstrap samples
            idx = _rng.choice(N_local - 10, size=min(n_pairs, N_local - 10),
                              replace=False)
            distances = []
            for i in idx:
                ...
                if i + 5 < N_local and j + 5 < N_local:
                    ...
                    if d0 > EPS_LYAPUNOV and d5 > EPS_LYAPUNOV:
    ...
    try:
        ...
        for s in range(len(surrs)):
            # P1-4.3 修复: 为每个 surrogate 派生独立子种子, 避免序列相关
            rate = _divergence_rate_metric(surrs[s], rng_seed=seed + s + 1)
        ...
        # P1-4.3 修复: real_rate 用固定子种子 (seed+1000) 避免与 surrogate 序列重叠
        real_rate = _divergence_rate_metric(data, rng_seed=seed + 1000)
...
def interpret_game_data(data_path_override: str = None,
                ...
                if 0 <= gi < len(df):
                    ...
                    for col in [c for c in df.columns if c in variables and c != var]:
                        try:
                            ...
                            # ROUND27 P2 修复: 原 `except: pass` 静默吞错, 现记录跳过原因
                            # (常见: 非数值列、索引越界、NaN 格式化失败), 便于审计回溯.
                            extras.append(f"{col}=<unavailable: {type(_col_err).__name__}>")
    ...
    for v in available_variables:
        ...
        half_life = np.log(2) / (1 - ev + EPS_LYAPUNOV) if ev < 1 else float('inf')
