...
> **DOC-05 设计选择说明 (2026-07-28)**:
> 本 Skill 选择"50% 丢弃"而非 Bai-Perron 结构断点检验, 是基于以下显式权衡:
...
>
> 此设计选择的代价是**信息损失** (50% 数据被丢弃而非自适应分割), 在 N≥500 的生产场景应升级为 Bai-Perron。
> 参见 `src/edm_auditor.py` 中的 SVD 残差监测实现, 以及 `tests/test_svd_residual_monitor.py` 的回归测试。
...
| 分析类型 | 校正 | 理由 |
|---------|------|------|
| EXPLORATORY | Benjamini-Hochberg FDR (q=0.05) | 允许 10% 假发现以保留统计效力 |
...
function route_rules(n, k_vars, is_binary, goal, has_prereg):
    ...
    //   has_prereg=False → Benjamini-Hochberg FDR (q=0.05)
...
| S13 Multi-Comp | α=0.05; Bonferroni α/K 公式; FDR 框架 (Benjamini-Hochberg 1995); P(FP) 概率恒等式 | FDR q=0.05 | ≥5 对 WARN; >1 对路由 |
