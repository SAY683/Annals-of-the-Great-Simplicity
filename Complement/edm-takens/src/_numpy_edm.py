...
def simplex_predict(series, E, lib, pred, tau=1, Tp=1):
    ...
    for i in range(pred_vec_start, pred_vec_end):
        ...
        # Truncate weights to match futures
        w = w[:len(future_vals)]
        w = w / w.sum()

        pred_val = np.dot(w, future_vals)
        obs_pos = i + E - 1 + Tp
        if obs_pos < n:
...
def SMapPredictNonlinear(series, E, Tp=1, tau=1, lib=None, pred=None,
    ...
    for theta in theta_range:
        ...
        for i in range(pred_vec_start, pred_vec_end):
            ...
            X_design = np.array(X_design[:len(y_target)])
            y_target = np.array(y_target)
            w_design = w[:len(y_target)]
...
def CCM(series_cause, series_effect, E, Tp=0, tau=1,
    ...
    for lib_size in lib_sizes:
        ...
        rhos = []
        for _ in range(min(sample, 10)):
            lib_idx = np.sort(np.random.choice(N, size=lib_size, replace=False))
