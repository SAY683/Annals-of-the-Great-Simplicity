...
def test_environment():
    with test("Layer 0: Environment Validation"):
        ...
        try:
            ...
            check(False, "pyEDM not installed (optional, some features disabled)")
...
def test_auditor():
    with test("Layer 2: Auditor Firewall — Blocking invalid configs"):
        ...
        check(r_ccm.status == 'PASS', f"CCM direction: {r_ccm.status}")
...
def test_cross_validation():
    with test("Layer 3: Cross-Validation + Verification Suite"):
        ...
        df = pd.read_csv(os.path.join(_SKILL_ROOT, 'data', 'game_log.csv'))
